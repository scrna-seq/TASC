...
void ParsedData::parseAndEstimateHyperParams(CLIArgs runningParams) {
	...
	if (!abkt_file_name.empty()){
		timed_log("abkt_file_name not empty. entering debug mode. ");
		struct stat buffer;
...
//        this->beta[k] = this->beta[k] + sizeFactors[k];
//        this->tau[k] = this->tau[k] + sizeFactors[k];
//    }
...
//											  runningParams.getMinRelTolCubature()
//	);
////	this->marginalLikelihoodArguments.displayAll();
//
...
//	minex_func.n = 5;
//	minex_func.f = &init_marginal_lklhood;
//	minex_func.params = (void *)&(this->marginalLikelihoodArguments);
//
...
//	int status;
//	double size;
//	//double epsabs = pow(std::numeric_limits<double>::epsilon(),0.25L);
//	double epsabs = runningParams.getMinRelTolErcc();
//	do
//	{
//		iter++;
...
//		verbose_timed_log("Iteration Number:\t"+std::to_string((long long)iter));
//		verbose_timed_log("Iteration Status:\t"+std::to_string((long long)status));
//		verbose_timed_log("Iteration Size:\t"+std::to_string((long double)size));
//
//	} while (status == GSL_CONTINUE && iter < runningParams.getMaxIterErcc());
//
//
//	// check if converged, and display corresponding information.
...
//
//	if (!converged){
//		timed_log("The simplex optimization algorithm is not converged. Accuracy of downstream estimations may be affected. Increase the ");
//	} else {
//		timed_log("The simplex optimization algorithm is converged.");
//	}
//
//
//	this->estimatedHyperParamValues.e_kappa = gsl_vector_get(s->x,0);
//	this->estimatedHyperParamValues.e_tau = gsl_vector_get(s->x,1);
//	this->estimatedHyperParamValues.sd_kappa = exp(gsl_vector_get(s->x,2));
//	this->estimatedHyperParamValues.sd_tau = exp(gsl_vector_get(s->x,3));
//	this->estimatedHyperParamValues.rho_kappa_tau = transform_rho_from_real_domain(gsl_vector_get(s->x,4));
//
//	timed_log("EK = "+std::to_string((long double)this->estimatedHyperParamValues.e_kappa));
//	timed_log("ET = "+std::to_string((long double)this->estimatedHyperParamValues.e_tau));
//	timed_log("SD_K = "+std::to_string((long double)this->estimatedHyperParamValues.sd_kappa));
...
bool ParsedData::isEqualSampleSize() {
	...
	//std::cout << this->xParser.getNumSamples() << "\t" <<  this->yParser.getColDim() << "\t" <<  this->erccParser.getNumSamples() << std::endl;
...
#pragma omp parallel for schedule(dynamic,1)
	...
	for(size_t i = 0; i<sample_size;++i){
		if(isSampleFailed[i]){
			...
			verbose_timed_log("Sample #"+std::to_string((long long) i)+" E_KAPPA:"+std::to_string((long double) val_kappa[i]));
			verbose_timed_log("Sample #"+std::to_string((long long) i)+" E_TAU:"+std::to_string((long double) val_tau[i]));
			verbose_timed_log("Sample #"+std::to_string((long long) i)+" MARGINAL:"+std::to_string((long double) val_marginal[i]));
	...
	failureSampleMsgBuilder << failedSampleIdx.size() << " samples failed in estimating Kappa and Tau. Their indexes are: ";
